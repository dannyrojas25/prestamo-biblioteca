...
public class JwtGenerador {
    ...
    public String generarToken(Authentication authentication) {
        ...
        Instant instantActual = Instant.now(); // Obtiene la fecha y hora actual en formato de tiempo universal coordinado (UTC)
        Instant instantExpiracion = instantActual.plusSeconds(Long.parseLong(jwtExpirationToken)); // Añade la duración del token a la fecha actual
        Date expiracionToken = Date.from(instantExpiracion); // Convierte el Instant en un Date (si es necesario)
        ...
        return Jwts.builder() //Construimos un token JWT llamado token
                ...
                .signWith(SignatureAlgorithm.HS512, jwtFirma) /*Utilizamos este método para firmar
    ...
    public String obtenerUsernameDeJwt(String token) {
        Claims claims = Jwts.parser() // El método parser se utiliza con el fin de analizar el token
                .setSigningKey(jwtFirma)// Establece la clave de firma, que se utiliza para verificar la firma del token
